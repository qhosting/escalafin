...
export class ScheduledTasksService {
  ...
  // Escanear alertas PLD en todos los tenants activos
  // Llamar diariamente (ej: 01:00 hrs) desde el scheduler externo
  async scanPldAlerts(): Promise<void> {
    try {
      console.log('--- Iniciando escaneo PLD de alertas ---');
      const { default: PldAlertsService } = await import('./pld-alerts');
      ...
      for (const tenant of tenants) {
        try {
          const service = new PldAlertsService(tenant.id);
          const result  = await service.scanForAlerts();
          if (result.totalAlertsCreated > 0) {
            console.log(`[PLD] ${tenant.name}: ${result.totalAlertsCreated} alertas nuevas.`);
          }
        } catch (err) {
          console.error(`[PLD] Error en tenant ${tenant.id}:`, err);
        }
      }

      console.log('--- Fin del escaneo PLD ---');
    } catch (error) {
      console.error('[PLD] Error crítico en escaneo:', error);
    }
  }

  // Drenar la cola CONDUSEF al inicio del período permitido (07:00 hrs)
  async processConductsefQueue(): Promise<void> {
    try {
      console.log('[CONDUSEF] Procesando cola de mensajes diferidos...');
      const { ConductsefTimeGuard } = await import('./condusef-time-guard');

      // Registrar ejecutores por tipo de job
      const executors: Record<string, (payload: Record<string, unknown>) => Promise<void>> = {
        'PAYMENT_REMINDER': async (payload) => {
          const service = new WhatsAppNotificationService(String(payload.tenantId ?? ''));
          await service.sendPaymentReminderNotification(String(payload.loanId));
        },
        'CUSTOM_MESSAGE': async (payload) => {
          const service = new WhatsAppNotificationService(String(payload.tenantId ?? ''));
          await service.sendCustomMessage(String(payload.clientId), String(payload.message));
        },
      };

      const processed = await ConductsefTimeGuard.drainQueue(executors);
      console.log(`[CONDUSEF] ${processed} mensajes procesados desde la cola.`);
    } catch (error) {
      console.error('[CONDUSEF] Error procesando cola:', error);
    }
  }
}
